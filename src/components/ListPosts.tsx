...
const ListPost = ({ post }: Props) => {
    ...
    return (
        <li className='grid box-border my-12 items-center grid-cols-2 gap-4 max-sm:grid-flow-row max-[1200px]:grid-cols-1 max-md:gap-12 font-Montserrat'>
            ...
            {description && (
                <div>
                    <a href={post.url || '404'} className='cursor-pointer hover:underline hover:underline-offset-4 hover:decoration-cyan-300 dark:hover:decoration-brand-blue'>
                    ...
                    <p>Author: <a href={author} className='cursor-pointer hover:underline hover:underline-offset-4 hover:decoration-cyan-300'><span className='italic text-xl'>{author || 'unknown'}</span></a></p>
